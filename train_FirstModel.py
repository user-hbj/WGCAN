...
def main(args):
    ...
    for epoch in range(lastepoch,args.num_epoch):
        ...
        for i, databatch in enumerate(train_loader):
            ...
            # 计算 PSNR 和 SSIM
            single_psnr =  peak_signal_noise_ratio(Long_patch, long_raw)
            single_ssim =  structural_similarity_index_measure(Long_patch, long_raw)
        ...
        #             # 将第一阶段的输出传递给第二阶段模型
        #             long_raw,outputs = model(input_patch)
        ...
        #             max_val = torch.max(Long_patch)
        #             # print(f"Long_patch最小值为{min_val},最大值为{max_val}")
        #             loss1 = criterion(long_raw,Long_patch)
        #             print(f"loss1的值为{loss1}")
        #             loss2 = criterion(outputs, gt_patch)
        #             print(f"loss2的值为{loss2}")
        #             loss = loss1 + loss2
...
if __name__ == "__main__":
    ...
    parser.add_argument('--result_dir', type=str, default='/raid/hbj/Wave_GCC/result/whole_pipline/new_dwt_RCABS/whole_stage/TwoStages_L1_FirstModel')
    ...
    parser.add_argument('--num_epoch', type=int, default=4021)
