...
from models.whole_new_dwt_RCABS_Two_Stages_LongRaw import *
from models.whole_new_dwt_RCABS_Two_Stages_LongRaw_NextModel import *
...
def main(args):
    ...
    save_val_model = os.path.join(result_dir, 'val_model')  # 保存验证模型的目录
    if not os.path.exists(save_val_model):
        os.makedirs(save_val_model)
    
    save_bestmodel = os.path.join(result_dir, 'best_loss_model') # 保存图像的最好权重
    if not os.path.exists(save_bestmodel):
        os.makedirs(save_bestmodel)

    save_lastmodel = os.path.join(result_dir, 'last_model') # 保存图像的最后一轮权重
    ...
    # print(f"训练集的总个数为{len(trainset)}") # 161
    ...
    firstModel.load_state_dict(torch.load(args.model))
    ...
    for epoch in range(lastepoch,args.num_epoch):
        ...
        for i, databatch in enumerate(train_loader):
            ...
            long_raw = firstModel(input_patch)
            
            # 将第一阶段的输出传递给第二阶段模型
            outputs = model(long_raw)
            ...
            # print(f"loss1的值为{loss1}")
            # loss2 = criterion(outputs, gt_patch)
            # print(f"loss2的值为{loss2}")
            # loss = loss1 + loss2
            # print(f"loss的值为{loss}")
            G_opt.zero_grad()
            loss.backward()
            G_opt.step()
        ...
        if(epoch % 100 == 0):
            ...
            with torch.no_grad():  # 在验证过程中不需要计算梯度
                ...
                for i, databatch in enumerate(val_loader):
                    ...
                    # print(f"Long_patch最小值为{min_val},最大值为{max_val}")
                    loss = criterion(long_raw,Long_patch)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--result_dir', type=str, default='/raid/hbj/Wave_GCC/result/whole_pipline/new_dwt_RCABS/whole_stage/TwoStages_L1_FirstModel_NextSecond_NoDongjie')
    ...
    parser.add_argument('--model', type=str, default='/raid/hbj/Wave_GCC/result/whole_pipline/new_dwt_RCABS/whole_stage/TwoStages_L1_FirstModel/best_loss_model/bestmodel_0.02381610663922978_2212.pth')
